...
HRESULT Scene::initialiseSceneResources() {
	...
	Effect *renderClothEffect = new Effect(device, "Shaders\\cso\\basic_cloth_vs.cso", "Shaders\\cso\\basic_cloth_ps.cso", basicClothVertexDesc, ARRAYSIZE(basicClothVertexDesc));
	//shadowEffect = new Effect(device, "Shaders\\csgo\\basic_shadow_vs.cso", "Shaders\\cso\\basic_shadow_ps.cso", basicVertexDesc, ARRAYSIZE(basicVertexDesc));
	...
	treeTexture = new Texture(device, context, L"Resources\\Textures\\tree.tif");
	...
	//shadow stuff
	quad = new Quad(device, basicTextureEffect->getVSInputLayout());

	D3D11_TEXTURE2D_DESC		depthStencilDesc;
	ID3D11Texture2D *shadowMapDepthStencilBuffer = nullptr;
	depthStencilDesc.Width = shadowMapSize;
	depthStencilDesc.Height = shadowMapSize;
	...
	HRESULT hr = device->CreateTexture2D(&depthStencilDesc, 0, &shadowMapDepthStencilBuffer);
	...
	hr = device->CreateDepthStencilView(shadowMapDepthStencilBuffer, &descDSV, &shadowMapDSV);
	...
	hr = device->CreateShaderResourceView(shadowMapDepthStencilBuffer, &shaderResourceViewDesc, &shadowMapSRV);
	// Release un-needed references
	if (shadowMapDepthStencilBuffer)
		shadowMapDepthStencilBuffer->Release();
	...
	ground = new Terrain(context, heightMap->getTexture(), normalMap->getTexture(), device, grassEffect, NULL, 0, grassTextureArray, 3);
	...
	ID3D11RasterizerState* shadowRSState = reflectionMapEffect->getRasterizerState();
	...
	reflectionMapEffect->setRasterizerState(shadowRSState);

	shadowRSState = treeEffect->getRasterizerState();
	shadowRSState->GetDesc(&RDesc);
	RDesc.DepthBias = 100;
	RDesc.DepthBiasClamp = 0.0f;
	RDesc.SlopeScaledDepthBias = 1.0f;
	device->CreateRasterizerState(&RDesc, &shadowRSState);
	treeEffect->setRasterizerState(shadowRSState);

	shadowRSState = skinningEffect->getRasterizerState();
	shadowRSState->GetDesc(&RDesc);
	RDesc.DepthBias = 100;
	RDesc.DepthBiasClamp = 0.0f;
	RDesc.SlopeScaledDepthBias = 1.0f;
	device->CreateRasterizerState(&RDesc, &shadowRSState);
	skinningEffect->setRasterizerState(shadowRSState);

	static const int numLights = 2;

	quadCBufferCPU = (CBufferModel *)_aligned_malloc(sizeof(CBufferModel), 16);

	quadCBufferCPU->worldMatrix = (XMMatrixScaling(0.2, 0.2, 0.2)*XMMatrixTranslation(0.8,0.8,0));
	...
	cBufferLightCPU[0].lightAmbient = XMFLOAT4(0.2, 0.2, 0.2, 1.0);
	...
	cBufferShadowCPU->shadowTransformMatrix = lightCamera->getViewMatrix() * lightCamera->getProjMatrix()* T;
	...
	cbufferInitData.pSysMem = quadCBufferCPU;// Initialise GPU CBuffer with data from CPU CBuffer

	hr = device->CreateBuffer(&cbufferDesc, &cbufferInitData,
		&quadCBufferGPU);
	mapCbuffer(context, quadCBufferCPU, quadCBufferGPU, sizeof(CBufferModel));
	context->VSSetConstantBuffers(2, 1, &quadCBufferGPU);




	cbufferDesc.ByteWidth = sizeof(CBufferShadow);
	cbufferInitData.pSysMem = cBufferShadowCPU;
	hr = device->CreateBuffer(&cbufferDesc, &cbufferInitData,
			&cBufferShadowGPU);
	mapCbuffer(context, cBufferShadowCPU, cBufferShadowGPU, sizeof(CBufferShadow));
	context->VSSetConstantBuffers(6, 1, &cBufferShadowGPU);
...
HRESULT Scene::renderScene() {
	...
	lightCamera->update(context);

	//render shadows
	ID3D11RenderTargetView *tempRT[1] = { 0 };
	context->OMSetRenderTargets(1, tempRT, shadowMapDSV);
	context->RSSetViewports(1, &shadowViewport);
	...
	//context->OMSetRenderTargets(1, &renderTargetView, system->getDepthStencil());
	context->RSSetViewports(1, &viewport);
	tempRT[0] = system->getBackBufferRTV();
	...
	if (quad)
	{
		...
		context->VSSetConstantBuffers(0, 1, &quadCBufferGPU);
		basicTextureEffect->bindPipeline(context);
		ID3D11ShaderResourceView *quadTextures[1] = { shadowMapSRV };
		context->PSSetShaderResources(0, 1, quadTextures);
		quad->render(context);
		quadTextures[0] = { nullptr };
		context->PSSetShaderResources(0, 1, quadTextures);
	}
	...
	{
		DrawGrass(context);
		ID3D11ShaderResourceView *nullTextures[1] = { nullptr };
		context->PSSetShaderResources(2, 1, nullTextures);
	}
...
HRESULT Scene::renderShadow(ID3D11DeviceContext *context) {
